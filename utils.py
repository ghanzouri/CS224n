...
def pad_sents(sents, pad_token):
    ...
    """
    # sents_padded = []
    sents_padded = [None] * len(sents)

    ### YOUR CODE HERE (~6 Lines)
    longest_len = len(sents[0])
    for i in range(len(sents)):
        if len(sents[i])>=longest_len:
            longest_len = len(sents[i])
            
    for i in range(len(sents)):
        if len(sents[i])!=longest_len:
            num_pad = longest_len - len(sents[i])
            sents_padded[i] = sents[i] + num_pad*[pad_token]
        else:
            sents_padded[i] = sents[i]
    ### END YOUR CODE
