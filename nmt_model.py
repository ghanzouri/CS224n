...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # print("TEEEST", self.model_embeddings.source)
        # print(source_padded)
        # print(source_lengths)
        src_len = source_padded.shape[0]
        ...
        for i in range(src_len):
            for j in range(b):
                X[i,j,:] = self.model_embeddings.source(source_padded[i,j])

        # print("XXXX", X.shape)
        # X = self.model_embeddings.source(source_padded)
        X_ = pack_padded_sequence(X, source_lengths)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X_)
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens)
        # print("enc_hidden",enc_hiddens.shape)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)
        # print("enc_hidden2",enc_hiddens.shape)

        # print("lasthidden", last_hidden[0].shape)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        @param enc_hiddens (Tensor): Hidden states (b, src_len, h*2), where
                                     b = batch size, src_len = maximum source sentence length, h = hidden size.
        @param c (Tensor): Tensor of sentence masks (b, src_len), where
        ...
        # print("AAAAAA", enc_hiddens_proj.shape)

        tgt_len = target_padded.shape[0]
        e = self.model_embeddings.embed_size
        Y = torch.zeros(tgt_len, batch_size, e)
        # print("test", (batch_size,b))

        for i in range(tgt_len):
            for j in range(batch_size):
                Y[i,j,:] = self.model_embeddings.target(target_padded[i,j])
        ...
        for Y_t in torch.split(Y, 1):
            ...
            (cell, state), o_t, _ = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
